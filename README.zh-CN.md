...
**每个 Topic 背后是一节课。** 路线只列 Topic *名字*;`course lesson`(或 TUI 里按 `p`)
让"大脑"把它展开成一节**预习讲义**——一段概述 + 3~5 道递进难度的**题目**,每题配完整
**题解**(思路、步骤、代码、复杂度)。讲义按需生成并缓存进 `<dir>/.tutor/lessons/<id>.md`
(可编辑 Markdown、随目录走、git 友好),二次打开秒出、不再调 omp;TUI 里 `s` 可把题解
藏起来自测,`g` 重新生成。
...
3. 按 **`p`** 打开这个课题的**讲义**:一段概述,然后 `题目 1..N`,每题后面跟着它的 `题解`。
4. 讲义里:`↑↓` / `jk` 滚动 · **`s`** 把所有题解藏起来(先自己做),再按 `s` 显示 · **`g`**
   用 `omp -p` 重新生成 · **`esc`** 返回看板。
...
讲义里:`↑↓`/`jk` 滚动 · **`s` 显示/隐藏题解** · `g` 重新生成(omp -p)· `esc` 返回。
